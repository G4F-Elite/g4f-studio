...
"""
Colab helpers for g4f Studio. Uses Colab's built-in proxy.
"""
...
def show_link(port: int = 8888, *, _url: "str | None" = None):
    ...
    *_url* is an optional pre-fetched proxy URL; pass it to avoid a second eval_js round-trip.
    ...
    logger.info(f"🌐 g4f Studio URL: {url}")
...
<div style="font-family:system-ui,-apple-system,sans-serif;margin:8px 0;
  ...
  <div style="display:flex;align-items:center;gap:10px;padding:10px 16px;background:#000;">
    ...
    <span style="color:#fff;font-weight:700;font-size:15px;letter-spacing:-0.2px;">g4f Studio</span>
...
def start(port: int = 8888):
    """
    Start g4f Studio server in Colab and display the URL.
    ...
    logger.info("🦥 Starting g4f Studio...")
    ...
    try:
        ...
        logger.error(f"❌ g4f Studio failed to start: {exc}")
        return
    ...
    if not server_ready:
        logger.error(
            f"❌ g4f Studio did not become healthy on port {actual_port}. "
