...
class InferenceBackend:
    ...
    def generate_chat_completion_with_tools(
        ...
        * ``{"type": "tool_end", "tool_name", "tool_call_id", "result"}``
        ...
        # Resolve max_tokens: prefer max_tokens, fall back to max_completion_tokens, then max_new_tokens
        _resolved_max_tokens = (
            max_tokens if max_tokens is not None
            else max_completion_tokens if max_completion_tokens is not None
            else max_new_tokens
        )
        ...
        def _single_turn(conv: list, *, active_tools: Optional[list[dict]] = None):
            ...
            yield from self._generate_chat_response_inner(
                ...
                top_k = top_k,
                min_p = min_p,
                max_new_tokens = _resolved_max_tokens,
    ...
    def _generate_vision_response(
        ...
        try:
            ...
            if typical_p is not None:
                generation_kwargs["typical_p"] = typical_p
            if mirostat is not None:
                generation_kwargs["mirostat_mode"] = mirostat
            if mirostat_tau is not None:
                generation_kwargs["mirostat_tau"] = mirostat_tau
            if mirostat_eta is not None:
                generation_kwargs["mirostat_eta"] = mirostat_eta
