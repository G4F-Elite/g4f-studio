...
_smart_apt_install() {
    ...
    if command -v sudo >/dev/null 2>&1; then
        ...
        case "$REPLY" in
            [nN]*)
                echo ""
                echo "    Please install these packages first, then re-run g4f Studio setup:"
        ...
        echo "    Please install these packages as root, then re-run g4f Studio setup:"
...
if [ -t 1 ]; then
    ...
    echo "g4f Studio did not become healthy within ${TIMEOUT_SEC}s." >&2
...
DESKTOP_EOF
        ...
        # ── macOS: .app bundle ──
        _css_app="$HOME/Applications/g4f Studio.app"
...
STUB_EOF
        ...
        # Symlink on Desktop
        if [ -d "$HOME/Desktop" ]; then
            ln -sf "$_css_app" "$HOME/Desktop/g4f Studio" 2>/dev/null || true
        fi
        _css_created=1
        ...
        # install's "g4f Studio.lnk" in the same folder. Per-distro suffix.
        if [ -n "$_css_distro" ]; then
            _css_lnk_name="g4f Studio (WSL - ${_css_distro}).lnk"
        else
            _css_lnk_name="g4f Studio (WSL).lnk"
        fi
...
\$iconDir = Join-Path \$env:LOCALAPPDATA 'g4f Studio'
...
foreach (\$dir in \$locations) {
    ...
    \$shortcut.Description = 'Launch g4f Studio (WSL)'
...
if [ -x "$VENV_DIR/bin/python" ]; then
    ...
    if [ "$_STUDIO_HOME_REDIRECT" = "env" ] \
       ...
       && [ ! -f "$STUDIO_HOME/bin/unsloth" ]; then
        echo "ERROR: $VENV_DIR already exists but does not look like an g4f Studio install." >&2
...
if [ -t 1 ]; then
    echo ""
    printf "  Start g4f Studio now? [Y/n] "
    if [ -r /dev/tty ]; then
    ...
    case "${_reply:-y}" in
        [Yy]*|"")
            step "launch" "starting g4f Studio..."
            ...
            if [ "$_LAUNCH_EXIT" -ne 0 ] && [ "$_MIGRATED" = true ]; then
                echo ""
                echo "⚠️  g4f Studio failed to start after migration."
